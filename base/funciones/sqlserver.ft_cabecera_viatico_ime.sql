	...
	***********************************/
	...
	if(p_transaccion='SQL_CABVI_INS')then
        ...
        begin
            ...
            v_id_int_comprobante =   conta.f_gen_comprobante (v_id_cabecera_viatico,'DEVPAGVIA',v_id_usuario_reg,NULL,NULL, NULL); 
			
                        
             if (pxp.f_get_variable_global('sincronizar') = 'true') then
            	
                v_conexion = migra.f_crear_conexion();               
                SELECT dblink_exec(v_conexion, 'update sci.tct_comprobante
                								set pedido = ''' ||v_parametros.nro_sigma ||''',
                                                aprobacion = ''VIATICOS'',
                                                conformidad = ''VIATICOS''
                                                where id_int_comprobante = '||v_id_int_comprobante ,true) into v_respu;
                v_respu = migra.f_cerrar_conexion(v_conexion,'exito');
		...
		begin
			...
			if (pxp.f_get_variable_global('sincronizar') = 'true') then
                ...
                'SELECT *
                  ...
                  (select id_comprobante from sci.tct_comprobante where id_int_comprobante=' || v_parametros.id_int_comprobante || ')
                  ,NULL,NULL
            	...
            	into v_respu; 
                if (split_part(v_respu, '#@@@#',1)='f') then
                	raise exception '%',split_part(v_respu, '#@@@#',2);
                end if;
                --raise exception '%',v_respu;
                v_respu = migra.f_cerrar_conexion(v_conexion,'exito');
            end if;
